...
export const DragDropEngine = {
    init(userState, currentItems, onMutationComplete) {
        ...
        if (canvas.classList.contains('view-freeform')) {
            this.initFreeformInteractions(canvas);
            return;
        }
    ...
    initFreeformInteractions(canvas) {
        ...
        const clampSize = (w, h) => ({
            w: Math.min(FREEFORM_MAX_W, Math.max(FREEFORM_MIN_W, w)),
            h: Math.min(FREEFORM_MAX_H, Math.max(FREEFORM_MIN_H, h))
        });
        ...
        const onResizeUp = () => {
            ...
            UI.saveFreeformSize(
                card.dataset.id,
                parseFloat(card.style.width) || FREEFORM_DEFAULT_W,
                parseFloat(card.style.height) || FREEFORM_DEFAULT_H
            );
            resizeActive = null;
            ...
            card.addEventListener('mousedown', (e) => {
                ...
                if (resizeHandle) {
                    ...
                    resizeActive = {
                        ...
                        origW: parseFloat(card.style.width) || FREEFORM_DEFAULT_W,
                        origH: parseFloat(card.style.height) || FREEFORM_DEFAULT_H
                    };
                ...
                if (!e.target.closest('.ff-grab-strip')) return;
